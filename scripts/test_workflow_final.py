"""Test script for the complete AuditChain workflow (v5) with 5 agents.

Executes the full pipeline:
Collector -> Reconciler -> Quant Analyst -> Investigator -> Supervisor

Usage:
    python -m scripts.test_workflow_final
"""

import time
import uuid
import sys
import traceback
from typing import Any

from rich.console import Console, Group
from rich.panel import Panel
from rich.table import Table
from rich.prompt import Prompt
from rich.text import Text

from auditchain.core.config import get_settings
from auditchain.core.logging import configure_logging, get_logger
from auditchain.graph.workflow import build_audit_graph
from auditchain.graph.state import create_initial_state
from auditchain.schemas.enums import AuditPhase
from auditchain.data.database import get_session
from auditchain.data.audit_repository import AuditRepository

logger = get_logger(__name__)
console = Console()
settings = get_settings()


def run_audit(graph: Any, test_name: str, cik: str, ticker: str | None) -> dict:
    """Executes the complete audit workflow (5 nodes) for a company."""
    console.rule(f"[bold cyan]{test_name}[/bold cyan]", style="cyan")
    console.print(f"[yellow]CIK:[/yellow] {cik}, [yellow]Ticker:[/yellow] {ticker}")

    run_id = str(uuid.uuid4())
    initial_state = create_initial_state(
        audit_run_id=run_id, 
        company_cik=cik, 
        company_ticker=ticker
    )
    
    start_time = time.time()
    try:
        console.print("[dim]Invoking AuditChain Graph (5 agents)...[/dim]")
        result = graph.invoke(initial_state)
        duration = time.time() - start_time
        
        cd = result.get("company_data")
        rec = result.get("reconciliation")
        quant = result.get("quant_analysis")
        inv = result.get("investigation")
        final = result.get("final_report")
        red_flags = result.get("red_flags", [])
        
        # 1. Company Info Panel
        company_info = []
        if cd:
            fraud_status = "[bold red]YES[/bold red]" if cd.is_known_fraud else "[green]NO[/green]"
            company_info.append(f"[bold]Company:[/bold] {cd.name} ({cd.ticker})")
            company_info.append(f"[bold]Known Fraud:[/bold] {fraud_status}")
            company_info.append(f"[dim]CIK: {cd.cik}[/dim]")
        
        # 2. Reconciliation Panel
        rec_info = []
        if rec:
            pass_color = "green" if rec.passed else "red"
            rec_info.append(f"[bold]Status:[/bold] [{pass_color}]{'PASSED' if rec.passed else 'FAILED'}[/{pass_color}]")
            rec_info.append(f"[bold]Red Flags:[/bold] {len(rec.red_flags)}")
        
        # 3. Quant Analysis Panel
        quant_info = []
        if quant:
            m_score = f"{quant.beneish_mscore:.4f}" if quant.beneish_mscore is not None else "N/A"
            z_score = f"{quant.altman_zscore:.4f}" if quant.altman_zscore is not None else "N/A"
            quant_info.append(f"[bold]Beneish M-Score:[/bold] {m_score} [dim]({quant.beneish_interpretation})[/dim]")
            quant_info.append(f"[bold]Altman Z-Score:[/bold]  {z_score} [dim]({quant.altman_interpretation})[/dim]")
            quant_info.append(f"[bold]Red Flags:[/bold]      {len(quant.red_flags)}")

        # 4. Investigation Panel
        inv_info = []
        if inv:
            evasive = "[red]YES[/red]" if inv.evasive_language_detected else "[green]NO[/green]"
            inv_info.append(f"[bold]Evasive Language:[/bold] {evasive}")
            inv_info.append(f"[bold]Related Parties:[/bold]  {len(inv.related_parties_detected)}")
            inv_info.append(f"[bold]Red Flags:[/bold]        {len(inv.red_flags)}")
            if inv.mdna_findings:
                inv_info.append(f"\n[bold]MD&A Findings:[/bold] [dim]{inv.mdna_findings[:300]}...[/dim]")

        # Display Intermediate Panels
        console.print(Panel(Group(*company_info), title="Company Details", border_style="blue"))
        console.print(Panel(Group(*rec_info), title="Phase 1: Reconciliation", border_style="magenta"))
        console.print(Panel(Group(*quant_info), title="Phase 2: Quantitative Analysis", border_style="cyan"))
        console.print(Panel(Group(*inv_info), title="Phase 3: Qualitative Investigation", border_style="green"))

        # 5. FINAL AUDIT REPORT
        if final:
            score = final.risk_score
            if score <= 20: score_color = "green"
            elif score <= 50: score_color = "yellow"
            elif score <= 75: score_color = "orange3"
            else: score_color = "red"
            
            human_review = "[bold red]YES[/bold red]" if result.get("needs_human_review") else "[green]NO[/green]"
            
            final_content = []
            final_content.append(f"[bold]Risk Score:[/bold] [{score_color}]{score}/100[/{score_color}]")
            final_content.append(f"[bold]Risk Level:[/bold] {final.risk_level.value.upper()}")
            final_content.append(f"[bold]Conclusion:[/bold] [underline]{final.audit_conclusion.value.upper()}[/underline]")
            final_content.append(f"[bold]Needs Human Review:[/bold] {human_review}\n")
            
            final_content.append("[bold underline]EXECUTIVE SUMMARY:[/bold underline]")
            final_content.append(final.executive_summary)
            
            final_content.append("\n[bold underline]RECOMMENDATIONS:[/bold underline]")
            for i, rec_item in enumerate(final.recommendations, 1):
                final_content.append(f"{i}. {rec_item}")
                
            console.print(Panel(Group(*final_content), title="[bold white]FINAL AUDIT REPORT[/bold white]", border_style="white", padding=(1, 2)))

        # 6. Consolidated Red Flags
        if red_flags:
            flags_table = Table(title="Consolidated Red Flags")
            flags_table.add_column("Agent", style="bold")
            flags_table.add_column("Severity", justify="center")
            flags_table.add_column("Title")
            
            for flag in red_flags:
                sev_color = "red" if flag.severity.value in ["critical", "high"] else "yellow"
                flags_table.add_row(
                    flag.detected_by,
                    f"[{sev_color}]{flag.severity.value.upper()}[/{sev_color}]",
                    flag.title
                )
            console.print(flags_table)

        # 7. Database Persistence Check
        persisted = "NO"
        try:
            with get_session() as session:
                repo = AuditRepository(session)
                run_record = repo.get_run(run_id)
                if run_record and run_record.status == "completed":
                    persisted = "[bold green]YES[/bold green]"
        except Exception as e:
            persisted = f"[red]ERROR ({str(e)})[/red]"
            
        console.print(f"\n[bold]Persisted to DB:[/bold] {persisted}")
        console.print(f"[bold]Execution Stats:[/bold] Duration: {duration:.2f}s | Tokens: {result.get('total_tokens', 0)} | Cost: ${result.get('total_cost_usd', 0.0):.4f}")
        
        return {
            "test_name": test_name,
            "phase": result.get("current_phase", AuditPhase.FAILED).value,
            "risk_score": final.risk_score if final else 0,
            "conclusion": final.audit_conclusion.value if final else "N/A",
            "flags": len(red_flags),
            "tokens": result.get("total_tokens", 0),
            "cost": result.get("total_cost_usd", 0.0),
            "duration": duration,
            "persisted": persisted == "[bold green]YES[/bold green]",
            "success": result.get("current_phase") == AuditPhase.COMPLETED
        }
        
    except Exception as e:
        console.print(f"[bold red]Exception during workflow execution:[/bold red]\n{traceback.format_exc()}")
        return {
            "test_name": test_name,
            "phase": "EXCEPTION",
            "risk_score": 0, "conclusion": "ERROR", "flags": 0,
            "tokens": 0, "cost": 0.0, "duration": 0, "persisted": False, "success": False
        }


def main():
    configure_logging()
    
    console.print("\n[bold reverse cyan] AuditChain: Full 5-Agent End-to-End Workflow (V5) [/bold reverse cyan]\n")
    
    # 1. Interactive Model Selection
    console.print("[bold yellow]Select LLM Model for Audit:[/bold yellow]")
    console.print("1) gpt-4o (Smartest, strict rate limits)")
    console.print("2) gpt-4o-mini (Faster, higher rate limits, recommended)")
    
    choice = Prompt.ask("Enter choice", choices=["1", "2"], default="2")
    model_choice = "gpt-4o" if choice == "1" else "gpt-4o-mini"
    
    settings.llm_smart_model = model_choice
    console.print(f"[bold green]Using model:[/bold green] {model_choice}\n")
    
    console.print("[dim]Compiling AuditChain Graph (Collector + Reconciler + Quant + Investigator + Supervisor)...[/dim]\n")
    graph = build_audit_graph()
    
    tests = [
        ("Full Audit: Bausch Health (BHC)", "0000885590", "BHC"),
        ("Full Audit: Apple (AAPL)", "0000320193", "AAPL"),
    ]
    
    results = []
    for i, (name, cik, ticker) in enumerate(tests):
        if i > 0:
            console.print(f"[yellow]Waiting 60 seconds to reset OpenAI Rate Limits (TPM)...[/yellow]")
            time.sleep(60)
            
        res = run_audit(graph, name, cik, ticker)
        results.append(res)
        console.print("\n")

    # Final Summary Table
    summary_table = Table(title="AuditChain Final Workflow Summary (V5)")
    summary_table.add_column("Test", style="cyan")
    summary_table.add_column("Conclusion", justify="center")
    summary_table.add_column("Risk Score", justify="right")
    summary_table.add_column("Flags", justify="right")
    summary_table.add_column("Persisted", justify="center")
    summary_table.add_column("Tokens", justify="right")
    summary_table.add_column("Cost (USD)", justify="right")
    summary_table.add_column("Duration (s)", justify="right")

    for res in results:
        p_mark = "[green]✓[/green]" if res["persisted"] else "[red]✗[/red]"
        summary_table.add_row(
            res["test_name"],
            res["conclusion"].upper(),
            str(res["risk_score"]),
            str(res["flags"]),
            p_mark,
            str(res["tokens"]),
            f"{res['cost']:.4f}",
            f"{res['duration']:.2f}"
        )

    console.print(summary_table)
    console.print(f"\n[bold cyan]End of Final Workflow Validation[/bold cyan]\n")


if __name__ == "__main__":
    main()
